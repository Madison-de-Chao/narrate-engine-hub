...
export const ShenshaStats = ({ shenshaList }: ShenshaStatsProps) => {
  ...
  return (
    <Card className="p-6 border-2 border-fuchsia-500/40 bg-gradient-to-br from-fuchsia-950 via-fuchsia-900/80 to-slate-900 relative overflow-hidden">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-emerald-400">{auspiciousCount}</div>
            <div className="text-xs text-muted-foreground flex items-center justify-center gap-1">
              <Shield className="w-3 h-3" /> 吉神
            </div>
          </div>
          <div className="bg-gradient-to-br from-rose-500/20 to-rose-500/5 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-rose-400">{ominousCount}</div>
            <div className="text-xs text-muted-foreground flex items-center justify-center gap-1">
              <AlertTriangle className="w-3 h-3" /> 凶煞
            </div>
          </div>
          <div className="bg-gradient-to-br from-violet-500/20 to-violet-500/5 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-violet-400">{specialCount}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-muted/20 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-foreground mb-4 text-center">神煞分類分布</h4>
          ...
          {/* 稀有度長條圖 */}
          <div className="bg-muted/20 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-foreground mb-4 text-center">稀有度分布</h4>
            <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {buffs.length > 0 && (
              <div className="bg-emerald-500/10 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-emerald-400 mb-2">增益效果 ↑</h4>
                <div className="flex flex-wrap gap-2">
                  {[...new Set(buffs)].map((buff, idx) => (
                    <Badge key={idx} variant="outline" className="text-emerald-400 border-emerald-400/30">
                      {buff}
                    </Badge>
                  ))}
                </div>
            ...
            {debuffs.length > 0 && (
              <div className="bg-rose-500/10 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-rose-400 mb-2">減益效果 ↓</h4>
                <div className="flex flex-wrap gap-2">
                  {[...new Set(debuffs)].map((debuff, idx) => (
                    <Badge key={idx} variant="outline" className="text-rose-400 border-rose-400/30">
                      {debuff}
                    </Badge>
                  ))}
                </div>
