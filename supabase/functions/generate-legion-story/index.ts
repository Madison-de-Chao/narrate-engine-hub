...
【官方角色名稱】
主將：${tgRole.role}（${stem}天干）
軍師：${dzRole.role}（${branch}地支）
納音戰場：${pillarData.nayin || '未知'}

【角色Buff/Debuff】
主將「${tgRole.role}」：強項「${tgRole.buff}」｜弱點「${tgRole.debuff}」
軍師「${dzRole.role}」：強項「${dzRole.buff}」｜弱點「${dzRole.debuff}」${bingfuSection}`;

    const systemPrompt = `你是「四時八字軍團戰記」專業故事作家。請依照《虹靈御所八字人生四時軍團秘笈》規範撰寫。

【核心原則】
這是故事，不是說明書。讀者要記得的是「兩位將軍在吵架」，不是「甲木=偏財」。
所有命盤術語必須藏在故事情節裡，由角色行為與對話自然帶出。

【三段式結構】（段落間空一行，嚴格遵守）

第一段｜場景＋角色登場（120-160字）
描述軍團所在地的視覺畫面，主將登場要有具體動作，展現人物特質。
首次提及角色用完整標註：「森林將軍（甲木天干）」，之後可用簡稱。

第二段｜事件展開＋技能展現（180-220字）
發生一個具體事件（戰役、談判、危機等）。
透過事件自然展現 Buff 和 Debuff 的效果（不直接說明）。
所有「兵符」必須在此段融入情節：貴人符=援軍來助、驛馬符=長途奔襲、華蓋符=獨處沉思。

第三段｜老兵對話＋收尾（120-160字）
用「老兵對新兵解釋」的對話格式，自然帶出一個術語的真實含義。
範例格式：老兵低聲說：「你看，將軍身上的偏財技能又發作了——一看到機會就忍不住...」
收尾用一句《孫子兵法》智慧（如「知己知彼，百戰不殆」）呼應故事主題。
